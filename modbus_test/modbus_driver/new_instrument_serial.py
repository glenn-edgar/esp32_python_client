...
#Input Status N* x 1 Byte
    ...
    def read_bits(self, modbus_address, registeraddress, bit_number,  functioncode=1 ):
        first_part  = self._numToOneByteString(modbus_address).encode() + self._numToOneByteString(functioncode).encode() + \
        ...
        if crc_flag != 0:
            ...
            byte_count = return_data[0]
            for i in return_data[1:]:
                if bit_number >= 8 :
                    bit_number -= 8
                    return_value.extend( self.unpack_bits( 8, i ) )
                else:
                    
                    return_value.extend( self.unpack_bits(bit_number, i ) )
                    bit_number = 0
    ...
    def write_bits(self, modbus_address, registeraddress, value, functioncode=15):
        ...
        for i in range(0, loop_count  ):
             if len( value ) > 8 :
                 ...
                 data = value                 
             bit_data.append(self.pack_bits( data )) 
             
        #print(bit_data)
        payloadToSlave =  self._numToOneByteString(modbus_address ).encode() + self._numToOneByteString(functioncode).encode() + \
                          self._numToTwoByteString(registeraddress) + self._numToTwoByteString(number_of_bits) + self._numToOneByteString( loop_count ).encode() +bit_data
...
#Error Check (LRC or CRC) 
    ...
    def write_registers(self, modbus_address, registeraddress, value, functioncode=16, signed=False):
        ...
        first_part  = self._numToOneByteString(modbus_address).encode() + self._numToOneByteString(functioncode).encode() + \
                        self._numToTwoByteString(registeraddress) + self._numToTwoByteString(register_number) +\
                        self._numToOneByteString( len(value)*2 ).encode()
...
#*N = FIFO Count
    ...
    def _calculateCrcString(self,inputstring):      # Constant for MODBUS CRC-16
      POLY = 0xA001
      ...
      for character in inputstring:
        ...
        register = self._XOR(register, character)
